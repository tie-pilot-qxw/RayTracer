...
impl BVH {
    ...
    fn new_p(
        ...
        if object_span == 1 {
            ...
            if comparator(&objects[0], &objects[1]) == Some(Ordering::Less) {
                left = objects[0].clone();
            ...
            objects.sort_by(|a, b| comparator(a, b).unwrap());
        ...
        if !left.bounding_box(0., 0., &mut box_left) || !right.bounding_box(0., 0., &mut box_right) {
            panic!("No bounding box in bvh_node constructor!");
        }

        let tree_box = AABB::surrounding_box(&box_left, &box_right);
        Self { left, right, tree_box }
    }
    ...
    fn box_x_compare(a: &Arc<dyn Hittable>, b: &Arc<dyn Hittable>) -> Option<Ordering> {
        Self::box_compare(a, b, 0)
    }

    fn box_y_compare(a: &Arc<dyn Hittable>, b: &Arc<dyn Hittable>) -> Option<Ordering> {
        Self::box_compare(a, b, 1)
    }

    fn box_z_compare(a: &Arc<dyn Hittable>, b: &Arc<dyn Hittable>) -> Option<Ordering> {
